...
class avl {
    ...
    private:
        ...
        void percorreEmOrdemAux (noh* umNoh, int& contador, string marca);
...
void avl::percorreEmOrdemAux (noh* umNoh, int& contador, string marca) {
    if (umNoh != NULL) {
        percorreEmOrdemAux (umNoh->esq, contador, marca);
        if (umNoh->elemento.marca == marca)
            contador += umNoh->elemento.quantidadeDisponivel;
        percorreEmOrdemAux (umNoh->dir, contador, marca);
    }
}
...
int avl::levantamento (string marca) {
    int cont = 0;
    percorreEmOrdemAux (raiz, cont, marca);
    return cont;
}
